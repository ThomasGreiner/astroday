...
const acosDeg = (value) => acos(value) * 360 / (2 * PI);
const asinDeg = (value) => asin(value) * 360 / (2 * PI);
const cosDeg = (value) => cos(value / 360 * (2 * PI));
const sinDeg = (value) => sin(value / 360 * (2 * PI));
const tanDeg = (value) => tan(value / 360 * (2 * PI));
...
function getDate(isSunrise, zenith, latitude, longitude, date = new Date()) {
  ...
  let rightAscension;
  rightAscension = 360 / (2 * PI) * atan(ascension);
